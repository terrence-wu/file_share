...
prev_models=glob(PRETRAINED+'/*.hdf5')
#print(isup_models)

ISUP_model_fn=grep('ISUP_.*_Lv%g\.'%work_lvl, prev_models, value=True)
ISUP_model_fn.sort()
print(ISUP_model_fn)

ISUP_model_fn=ISUP_model_fn[-1]
...
def tr_uuid2tiles5(uuid, verbose=False, image_lvl=None, forced=False, return_img=False, save_img=True):
        ...
        final=uuid2tiles5(uuid, input_dir=TRAIN_IMAGES, output1_dir=output1_dir, output0_dir=output0_dir, 
                    LRdir=TRLRB, TNdir=TRTNB, ref_csv=train_csv, save_img=save_img,
...
def te_uuid2tiles5(uuid, verbose=False, image_lvl=None, forced=False, return_img=True, save_img=False):
        gc.collect()
        gc.collect()
        if forced:
        ...
        final=uuid2tiles5(uuid, input_dir=TEST_IMAGES, output1_dir=output1_dir, output0_dir=output0_dir, 
                    LRdir=TELRB, TNdir=TETNB, ref_csv=test_csv, save_img=save_img,
                    testing_mode=True, verbose=verbose, return_img=return_img)
        if image_lvl is None or image_lvl==1:
...
ISUP_model=load_model(ISUP_model_fn)
...
if TEST_step2_flag:
    ...
    mkdir(TETNB)
    print(TEST_IMAGES)
    TEST_PRED_CSV="test_pred_Lv0.csv"
    w_uuids=test_uuids
    ...
    for iii in np.arange(iii, len(w_uuids)):
        ...
        try:
            ...
            nfinal=len(imgs0)
            if verbose:
                print("img0 =", str(imgs0.shape))
            if nfinal>0:
                #uuid_datagen=InferenceDataGenerator(X=imgs0/255.0, batch_size=16)
                #preds=ISUP_model.predict_generator(uuid_datagen)
                preds=ISUP_model.predict(imgs0/255.0)
                print(preds.shape)
